...
for line in coverage_output.split("\n"):
    parts = line.split()
    if len(parts) >= 6 and parts[5].endswith("%") and int(parts[5].strip("%")) < 50:
        file_path = parts[0]
        if file_path.endswith(".py") and "tests/" not in file_path:
...
if not uncovered_files:
    ...
    for file in uncovered_files:
        ...
        subprocess.run(["pynguin", "--project-path", ".", "--module-name", module_name, "--output-path", "tests_lessthan50"], capture_output=False)
